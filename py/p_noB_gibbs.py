...
# For P'
def initializeUnknownLabelsForGibbsSampling_biasVariance_3(G,label,testLabels,parameters,G1,label1):
	currentLabelEstimates = dict(label)
	currentLabelEstimates_oneLabel = dict(label)
	"""
	currentLabelEstimates = {}
	...
	if parameters == None:
		# Compute Parameters before making initial estimates
		classPrior, estimatedProbabities, classPriorCounts, estimatedCounts = computeInitialParameters(G1,label1,{})
	else:
		# Set the Parameters values determisnistically without computing based on te data to understand how the 
		# prediciton perrforms with a given parameter values
		classPrior0 = parameters[0]
...
def gibbsSampling_biasVariance_hoda(edges,label,testLabels,parameters,originalLabels,noOfLabelsToMask):
	...
	classPrior_temp,estimatedProbabities_temp,currentLabelEstimates_initial_temp,classPriorCounts_temp,estimatedCounts_temp = initializeUnknownLabelsForGibbsSampling_biasVariance_2(edges,label,testLabels,parameters,testLabels)
	...
	classPrior[0] = [0.01,0.99]
	classPrior[1] = [0.01,0.99]
	classPrior[2] = [0.01,0.99]
	...
	for times in range(5):
		...
		finalLabel = {}
		finalLabel_forSquaredLoss = {}
		for node in nodeTraversalOrder:
			t = 0
			for k in range(noOfEnsembles):
				t += resultingLabels[k][node]

			t = (t + 0.0)/noOfEnsembles
			finalLabel_forSquaredLoss[node] = t
			if t >= 0.5:
				t = 1
			else:
				t = 0			
		...
		# Compute Squared Loss with the averages of Gibbs sampling before assigning them a single value
		squaredLoss = computeSquaredLoss(label,testLabels,finalLabel_forSquaredLoss)
		...
		print "-----------------------------------------------------------------------------------------"
		print "-----------------------------------------------------------------------------------------"
		print currentLabelEstimates_oneLabel
		print "hereherer"
		raw_input('')
		"""
...
# Hoda's ensemble method
## Gibbs Sampling for doing bias variance analysis
# currentGraph,currentLabels - Noised versions of original graph

def gibbsSampling_biasVariance_pPrime(edges,label,testLabels,parameters,originalLabels,noOfLabelsToMask,currentGraph,currentLabels,noOfEnsembles):
	...
	for k in range(noOfEnsembles):
		#testLabels_temp = random.sample(originalLabels,noOfLabelsToMask)
		classPrior_temp,estimatedProbabities_temp,currentLabelEstimates_initial_temp,classPriorCounts_temp,estimatedCounts_temp = initializeUnknownLabelsForGibbsSampling_biasVariance_3(edges,label,testLabels,parameters,currentGraph[k],currentLabels[k])
		classPrior.append(classPrior_temp)
		estimatedProbabities.append(estimatedProbabities_temp)
		currentLabelEstimates_initial.append(currentLabelEstimates_initial_temp)
		classPriorCounts.append(classPriorCounts_temp)
