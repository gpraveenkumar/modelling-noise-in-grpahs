...
def makeNoisyGraphs(action,percentageOfGraph,noOfTimesToRepeat,originalGraph,originalLabels,testLabels,percentageOfGraph2):
	...
	onlyTrainGraph = dict(originalGraph)
	onlyTrainLabels = dict(originalLabels)
	...
	newGraph.append(onlyTrainGraph)
	newLabels.append(onlyTrainLabels)
...
def func_star_biasVariance_pPrime(a_b):
    """Convert `f([1,2])` to `f(1,2)` call."""
    return gibbsSampling_biasVariance_pPrime(*a_b)
		...
		for noOfTimesToRepeat in noOfTimesToRepeatList:
				...
				for i in range(200):
					...
					if performInfernceOnly:
						...
						arg_t = [originalGraph,originalLabels,testLabels,None,originalLabels,noOfLabelsToMask,currentGraph,currentLabels,noOfTimesToRepeat+1]	

					#noOfTimesToRepeat + 1 for the original graph
					#a,b,c = gibbsSampling_biasVariance_pPrime(originalGraph,originalLabels,testLabels,None,originalLabels,noOfLabelsToMask,currentGraph,currentLabels,noOfTimesToRepeat+1)
					arguments = []
					...
					pool = Pool(processes=noofProcesses)
					y = pool.map(func_star_biasVariance_pPrime, arguments)
					pool.close()
					pool.join()
					...
					for label in ylm:
						ylm[label] /= (len(learningPredictions) + 0.0)
					...
					for label in ytm:
						ytm[label] /= (len(totalPredictions) + 0.0)
				...
				outputTofile.append( [ Action + "pPrime_new_4.txt",prefix , str(trainingSize) , str(round(avgSquaredLoss,4)) , str(round(avgLearningBias,4)) , str(round(avgLearningVariance,4)) , str(round(avgTotalBias,4)) , str(round(avgTotalVariance,4)) , str(round(avgInferenceBias,4)) , str(round(avgInferenceVariance,4)) ])
