...
for i in range(n):
	for j in range(i+1,n):
		if data[i,j] == 1:
			t = []
			t.append(str(int(id_label_mapping[i])))
			t.append(str(int(id_label_mapping[j])))
			l.append(t)
			t = []
			t.append(str(int(id_label_mapping[j])))
			t.append(str(int(id_label_mapping[i])))
...
f = open(path + "attributeCorrelationCheck.txt", 'w')
f.write("A B"+'\n')
...
for i in range(n):
	if i in labelsToMask:
		continue
	for j in range(i+1,n):
		if data[i,j] == 1 and j not in labelsToMask:
			#print str(id_label_mapping[i])+" "+str(id_label_mapping[j])
			estimatedProbabities[ id_label_mapping[i] , id_label_mapping[j] ] += 1
			estimatedProbabities[ id_label_mapping[j] , id_label_mapping[i] ] += 1
