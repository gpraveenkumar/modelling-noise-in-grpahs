...
for trainingSize in trainingSizeList:
	...
	for i in range(200):
		...
		meanAccuracy,meanPrecision,meanRecall,meanSquaredLoss = MPLE(originalGraph,originalLabels,testLabels,nodeAttributes,mleParameters)
	...
	outputTofile.append( [str(trainingSize), str(round(meanSquaredLoss,4)) , str(round(meanAccuracy,4)) , str(round(sd,4)) , str(round(se,4)) , str(round(meanPrecision,4)) , str(round(meanRecall,4)) , str(round(f1,4)) ])
