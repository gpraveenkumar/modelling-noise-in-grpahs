...
def MPLE(G,label,testLabels,nodeAttributes,mleParameters):
	...
	return meanAccuracy,meanPrecision,meanRecall,meanSquaredLoss
	...
	for percentageOfGraph in percentageOfGraphList:
		...
		for noOfTimesToRepeat in noOfTimesToRepeatList:
			...
			for i in range(200):
				...
				meanAccuracy,meanPrecision,meanRecall,meanSquaredLoss = MPLE(currentGraph,currentLabels,testLabels,currentNodeAttributes,mleParameters)
			...
			outputTofile = []
			header = ['trainingSize','meanSquaredLoss','meanAccuracy','sd','se','meanPrecision','meanRecall','f1']
			#outputTofile.append(header)

			prefix = str(int(percentageOfGraph*100)) + "perc_" + str(noOfTimesToRepeat) + "repeat"
			outputTofile.append( [prefix, str(trainingSize), str(round(meanSquaredLoss,4)) , str(round(meanAccuracy,4)) , str(round(sd,4)) , str(round(se,4)) , str(round(meanPrecision,4)) , str(round(meanRecall,4)) , str(round(f1,4)) ])


		fileName = "flipLabel.txt"
		path = basePath + '../results/Rongjing-mple_noise-' + school + '-' + schoolLabel + '-' 
