	...
	for percentageOfGraph in percentageOfGraphList:
		...
		for noOfTimesToRepeat in noOfTimesToRepeatList:
			...
			for i in range(5):
				...
				trainingLabels = [i for i in currentLabels if i not in testLabels]
				#print "Start trainLabels:",len(trainingLabels)

				mleParameters,indepModel_accuracy,indepModel_sqauredLoss = independentModel(currentLabels,currentNodeAttributes,trainingLabels)
				mpleParameters,mpleModel_accuracy,initialAccuracy,mpleModel_squaredLoss = MPLE(currentGraph,currentLabels,testLabels,currentNodeAttributes,mleParameters)
				...
				for labeledProportion in numpy.arange(0.0,1.0,0.25):
					...
					ki_list = computeProgagationUpperBound(onlyTrainingG,currentLabels,trainingLabels,L,mpleParameters)
					#print len(ki_list)
				...
				for t in tau:
					for k0 in kappa:
						...
						for labeledProportion in numpy.arange(0.0,1.0,0.25):
							...
							# Note: currentLabelEstimate is assigned a probability value so that in actual gibbs sampling function, they can get assigned labels randomly.
							# Hopefully, that variation will help in getting better label predictions.
							mu,currentLabelEstimates,lamda_list = computeInitialEstimate(currentNodeAttributes,onlyTrainingG,L,finalTrainingLabels_test,currentLabels,mleParameters,mpleParameters,t,k0,ki_list)

							arg_t = [onlyTrainingG,currentLabels,finalTrainingLabels_test,currentNodeAttributes,currentLabelEstimates,mleParameters,mpleParameters,lamda_list]
				...
				#error = innerLoop_trainTestAlgo(trainingLabels,currentGraph,currentNodeAttributes,mleParameters,mpleParameters,t,k0)
				#print "Final Error", error
				
				#lSize = int(labeledProportion*len(trainingLabels))
				#L = random.sample(trainingLabels,lSize)
				#originalTestLabels = [node for node in currentLabels if node not in trainLabels]
				...
				#print len(currentGraph)
				ki_list = computeProgagationUpperBound(currentGraph,currentLabels,currentLabels,trainingLabels,mpleParameters)
				#print len(testLabels)
				#print len(ki_list)
				...
				# Note: currentLabelEstimate is assigned a probability value so that in actual gibbs sampling function, they can get assigned labels randomly.
				# Hopefully, that variation will help in getting better label predictions.
				mu,currentLabelEstimates,lamda_list = computeInitialEstimate(currentNodeAttributes,currentGraph,trainingLabels,testLabels,currentLabels,mleParameters,mpleParameters,t,k0,ki_list)
				""""
				print "********************************"
				...
				print "********************************"
				"""
				#accuracy,precision,recall = computeAccuracy(currentLabels,testLabels,currentLabelEstimates)
				#initialEstimate.append(accuracy)
				initialEstimate.append(0)

				#accuracy, precision, recall, squaredLoss = gibbsSampling(currentGraph,currentLabels,testLabels,currentNodeAttributes,currentLabelEstimates,mleParameters,mpleParameters,lamda_list)
				arg_t = [currentGraph,currentLabels,testLabels,currentNodeAttributes,currentLabelEstimates,mleParameters,mpleParameters,lamda_list]
